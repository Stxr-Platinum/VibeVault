...
import com.vibevault.app.extensions.currentMetadata
import com.vibevault.app.extensions.metadata
import com.vibevault.app.extensions.toMediaItem
import com.vibevault.app.models.MediaMetadata
import com.vibevault.app.models.MediaMetadata.Album
import com.vibevault.app.models.MediaMetadata.Artist
...
class ListenTogetherManager @Inject constructor(
    ...
    companion object {
        ...
        private const val SYNC_DEBOUNCE_THRESHOLD_MS = 300L
    ...
    // Emits track ID when a guest receives a track change — UI observes this to auto-open player
    private val _guestTrackChanged = MutableStateFlow<String?>(null)
    val guestTrackChanged = _guestTrackChanged.asStateFlow()
    
    fun clearGuestTrackChanged() {
        _guestTrackChanged.value = null
    }
    ...
    * Initialize event collection. Should be called once at app start.
                    ...
                    action.trackInfo?.let { track ->
                        ...
                        if (action.queue != null && action.queue.isNotEmpty()) {
                            ...
                            applyPlaybackState(
                                ...
                                bypassBuffer = true // Skip room buffering, just play when ready
                            )
                        } else {
    ...
    private fun applyPlaybackState(
        ...
        scope.launch(Dispatchers.Main) {
            ...
            try {
                ...
                // CRITICAL: Must call prepare() so ExoPlayer starts buffering, otherwise
                // it stays in STATE_IDLE and our bypassBuffer loop will timeout after 5 seconds.
                player.prepare()
    ...
    private fun MediaItem.toTrackInfo(): TrackInfo {
        val metadata = this.metadata ?: return TrackInfo("unknown", "Unknown", "Unknown", "", 0, "")
        val durationMs = if (metadata.duration > 0) metadata.duration.toLong() * 1000 else 180000L
        return TrackInfo(
            id = metadata.id,
            title = metadata.title,
