...
import com.vibevault.app.data.stats.ArtistFacts
import com.vibevault.app.data.stats.ListeningStats
import com.vibevault.app.data.stats.ReplayPeriod
import com.vibevault.app.data.stats.ReplaySummary
 ...
 *   page, the stories, the share sheet.
 */
@Composable
...
fun ReplaySummary.songRows(limit: Int): List<ReplayRow> =
    songs.take(limit).mapIndexed { index, entry ->
        ReplayRow(
            key = entry.song.id.ifBlank { "${entry.song.title}|${entry.song.artist}" },
            rank = index + 1,
            title = entry.song.title,
            subtitle = entry.song.artist.takeIf { it.isNotBlank() },
            artworkUrl = entry.song.albumImageUrl.takeIf { it.isNotBlank() },
            ms = entry.ms,
            plays = entry.plays,
        )
    }
...
fun ReplaySummary.cards(): List<ReplayHeroCard> = buildList {
    add(
        ReplayHeroCard(
            ...
            artworkUrl = songs.firstOrNull()?.song?.albumImageUrl?.takeIf { it.isNotBlank() },
    ...
    songs.firstOrNull()?.let {
        add(
            ReplayHeroCard(
                ...
                artworkUrl = it.song.albumImageUrl.takeIf { it.isNotBlank() },
